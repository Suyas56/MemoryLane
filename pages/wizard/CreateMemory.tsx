...
export const CreateMemory = () => {
  ...
  const [formData, setFormData] = useState<Partial<MemoryEvent>>(() => {
    const saved = localStorage.getItem('draftMemory');
    if (saved) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('draftMemory', JSON.stringify(formData));
  }, [formData]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const eventId = await storageService.createEvent(newEvent);
      
      // Clear draft
      localStorage.removeItem('draftMemory');
      // Navigate to view page to see the created memory
    ...
    <div className="flex-1 flex flex-col relative w-full">
       ...
       <div className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 px-4 md:px-6 py-4 flex items-center justify-between sticky top-0 z-30">
         <div className="flex items-center gap-2 md:gap-4">
            ...
            <span className="font-bold text-gray-900 dark:text-white text-sm md:text-base">Create Memory</span>
         </div>
           ...
           >
             ...
             {step === 4 && (
               <div className="space-y-6">
                 ...
                 <div className="flex justify-between"><button onClick={() => setStep(3)} className="text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">Back</button><button onClick={handleSave} disabled={loading} className="bg-green-600 text-white px-8 py-3 rounded-xl font-bold shadow-lg shadow-green-200 dark:shadow-none hover:bg-green-700 transition flex items-center gap-2">
